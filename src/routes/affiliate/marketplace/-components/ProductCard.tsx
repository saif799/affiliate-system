...
const statusConfig = {
  hot: { label: '🔥 رائج', className: 'bg-red-100 text-red-700' },
...
export function ProductCard({ product, onOpenModal }: Props) {
  ...
  return (
    ...
    >
      {/* Thumbnail */}
      <div className="relative flex h-36 items-center justify-center bg-gray-50 text-5xl border-b border-gray-100">
        {product.thumbnail}
      ...
      <div className="flex flex-1 flex-col gap-3 p-3.5">
        ...
        <div className="rounded-lg bg-violet-50 px-3 py-2 text-center">
          <p className="text-xs text-violet-500">عمولتك لكل طلبية</p>
          <p className="mt-0.5 text-lg font-bold text-violet-700">
            {product.commission.toLocaleString('ar-DZ')} د.ج
          </p>
          <p className="text-xs text-violet-400">{product.commissionRate}% من سعر البيع</p>
        </div>
        ...
        <div className="mt-auto flex items-center justify-between pt-1 border-t border-gray-100">
          <span className="text-xs text-gray-400">{product.totalSales.toLocaleString()} مبيعة</span>
          <button
