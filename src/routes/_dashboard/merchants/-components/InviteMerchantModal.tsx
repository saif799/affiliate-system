// src/routes/_dashboard/merchants/-components/InviteMerchantModal.tsx
...
interface Props {
  loading:  boolean
  onClose:  () => void
  onSubmit: (data: InviteMerchantInput) => void
}

const INITIAL: InviteMerchantInput = {
  name:         '',
  email:        '',
  phone:        '',
  businessName: '',
...
type FieldErrors = Partial<Record<keyof InviteMerchantInput, string>>

function validate(data: InviteMerchantInput): FieldErrors {
  ...
  if (!data.businessName.trim()) errors.businessName = 'اسم النشاط التجاري مطلوب'
  return errors
}

export function InviteMerchantModal({ loading, onClose, onSubmit }: Props) {
  const [form,   setForm]   = useState<InviteMerchantInput>(INITIAL)
  const [errors, setErrors] = useState<FieldErrors>({})

  function handleChange(field: keyof InviteMerchantInput, value: string) {
  ...
  const fields: { key: keyof InviteMerchantInput; label: string; type?: string; placeholder: string }[] = [
    ...
    { key: 'businessName', label: 'اسم النشاط التجاري',      type: 'text',  placeholder: 'مثال: متجر الأفق'          },
  ]
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-slate-800 text-sm">إضافة تاجر جديد</p>
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          <div className="flex items-start gap-2.5 bg-indigo-50 border border-indigo-100 rounded-xl p-3">
            <svg className="shrink-0 mt-0.5" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6366f1" strokeWidth="2">
              <circle cx="12" cy="12" r="10" />
              <line x1="12" y1="8" x2="12" y2="12" />
              <line x1="12" y1="16" x2="12.01" y2="16" />
            </svg>
